...
var politicians = ["Donald Trump", "Hillary Clinton", "Bernie Sanders", "Mitch McConnell", "Barack Obama", "Jeb Bush", "Paul Ryan"];
...
function displayGifs() {
	$('.gif-display').empty();
	//variable that controls how many gifs are returned
	var gifCount = 10;
	...
	var queryURL = "https://api.giphy.com/v1/gifs/search?q=" + userSubmit + "&limit=" + gifCount + "&api_key=6181b0ebaf0342f1b3f5f8e23474b3fe"
	...
	$.ajax({
		...
		//loop through the object creating a div, p(rating), and img(GIFs) for each index and append it to gif-display
		for (var i = 0; i < gifCount; i++) {
			var gifDiv = $('<div class="item">');
			var rating = response.data[i].rating;
			var p = $("<p class='rating-text'>").text("Rating: " + rating.toUpperCase());
			var gifImg = $('<img>');
			gifImg.addClass('gif-img').attr('src', response.data[i].images.fixed_height_still.url);;
			gifDiv.append(p);
...
// call renderButtons() when the page finishes loading
